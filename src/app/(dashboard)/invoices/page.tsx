...
export default function InvoicesPage() {
  ...
  function openPayModal() {
    ...
    const due = selectedInvoice.total - selectedInvoice.paid;
    setPayForm({ amount: due, method: "CASH", reference: "", notes: "" });
    setPayModal(true);
  }
  ...
  async function handlePay(e: React.FormEvent) {
    ...
    try {
      ...
      setPayModal(false);
      viewDetail(selectedInvoice.id); // refresh
      fetchData();
              ...
              columns={[
                ...
                { key: "status", label: "Status", render: (i) => <Badge className={getStatusColor(i.status)}>{getStatusLabel(i.status)}</Badge> },
                {
                  key: "actions", label: "", render: (i) => (
                    <Button variant="ghost" size="sm" onClick={() => viewDetail(i.id)}>Detail</Button>
                  ),
                },
              ]}
            />
        ...
        {selectedInvoice && (
          <Card>
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              <div className="flex justify-between"><span className="text-gray-500">No. WO</span><span>{selectedInvoice.workOrder?.orderNumber}</span></div>
              ...
              {selectedInvoice.status !== "PAID" && (
                <Button className="w-full mt-3" onClick={openPayModal}>Bayar</Button>
              )}
      ...
      <Modal open={payModal} onClose={() => setPayModal(false)} title="Pembayaran Invoice">
        <form onSubmit={handlePay} className="space-y-4">
          <div>
            ...
            <input type="number" value={payForm.amount} onChange={(e) => setPayForm({ ...payForm, amount: Number(e.target.value) })} required min={1} className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Metode</label>
            <select value={payForm.method} onChange={(e) => setPayForm({ ...payForm, method: e.target.value })} className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm">
          ...
          <div>
            ...
            <input value={payForm.reference} onChange={(e) => setPayForm({ ...payForm, reference: e.target.value })} className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm" placeholder="No. referensi / bukti" />
          </div>
