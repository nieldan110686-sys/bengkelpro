...
export default function InventoryPage() {
  ...
  const [form, setForm] = useState({ code: "", name: "", category: "", brand: "", unit: "pcs", stock: 0, minStock: 5, buyPrice: 0, sellPrice: 0, location: "", notes: "" });
  ...
  async function fetchData() {
    ...
    setItems(data.inventory); setTotal(data.total); setLoading(false);
  }

  function openCreate() { setEditId(null); setForm({ code: "", name: "", category: "", brand: "", unit: "pcs", stock: 0, minStock: 5, buyPrice: 0, sellPrice: 0, location: "", notes: "" }); setModalOpen(true); }
  function openEdit(item: InventoryItem) { setEditId(item.id); setForm({ code: item.code, name: item.name, category: item.category || "", brand: item.brand || "", unit: item.unit, stock: item.stock, minStock: item.minStock, buyPrice: item.buyPrice, sellPrice: item.sellPrice, location: "", notes: "" }); setModalOpen(true); }
        ...
        <CardHeader>
          <div className="flex gap-4 flex-wrap">
            <div className="relative flex-1 max-w-sm">
              ...
              <input type="text" placeholder="Cari nama, kode, kategori..." value={q} onChange={(e) => { setQ(e.target.value); setPage(1); }} className="pl-10 pr-4 py-2 w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-sm focus:ring-2 focus:ring-primary-500 outline-none" />
            </div>
        ...
        <CardContent>
          <Table loading={loading} data={items}
            columns={[
              ...
              { key: "stock", label: "Stok", render: (i) => (
                <span className={i.stock <= i.minStock ? "text-red-600 font-bold" : ""}>{i.stock} {i.unit}</span>
              )},
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editId ? "Edit Barang" : "Tambah Barang"} className="max-w-xl">
        <form onSubmit={handleSave} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            {[
              { label: "Kode Barang", field: "code", required: true },
              { label: "Nama Barang", field: "name", required: true },
              { label: "Kategori", field: "category" },
              { label: "Merek", field: "brand" },
              { label: "Satuan", field: "unit" },
              { label: "Stok Awal", field: "stock", type: "number" },
              { label: "Stok Minimum", field: "minStock", type: "number" },
              { label: "Harga Beli", field: "buyPrice", type: "number" },
              { label: "Harga Jual", field: "sellPrice", type: "number" },
      ...
      <Modal open={stockModal} onClose={() => setStockModal(false)} title={`Update Stok: ${selectedItem?.name}`}>
        <form onSubmit={handleStockUpdate} className="space-y-4">
          <div>
            ...
            <select value={stockForm.type} onChange={(e) => setStockForm({ ...stockForm, type: e.target.value })} className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm">
          ...
          <div>
            ...
            <input type="number" min={1} value={stockForm.quantity} onChange={(e) => setStockForm({ ...stockForm, quantity: Number(e.target.value) })} required className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Catatan</label>
            <input value={stockForm.notes} onChange={(e) => setStockForm({ ...stockForm, notes: e.target.value })} className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm" />
          </div>
