...
export default function WorkOrdersPage() {
  ...
  async function fetchVehicles() { const res = await fetch("/api/vehicles?limit=100"); setVehicles((await res.json()).vehicles); }
  async function fetchMechanics() { const res = await fetch("/api/mechanics"); setMechanics(await res.json()); }

  function openCreate() { setEditId(null); setForm({ vehicleId: "", customerName: "", customerPhone: "", mechanicId: "", description: "", notes: "", mileage: "" }); setModalOpen(true); }
        ...
        <CardContent className="p-0">
          <Table loading={loading} data={workOrders}
            columns={[
              ...
              { key: "customer", label: "Pelanggan", render: (wo) => wo.vehicle.customer.name },
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              <div key={field}>
                ...
                <input value={(form as any)[field]} onChange={(e) => setForm({ ...form, [field]: e.target.value })} required={required} className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500 outline-none" />
              </div>
            ))}
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Mekanik</label>
            <select value={form.mechanicId} onChange={(e) => setForm({ ...form, mechanicId: e.target.value })} className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm">
              <option value="">Pilih Mekanik</option>
              {mechanics.map((m) => <option key={m.id} value={m.id}>{m.name} ({m.role})</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Deskripsi / Keluhan</label>
            <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} required rows={3} className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500 outline-none" />
          </div>
