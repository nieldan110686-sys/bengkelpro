...
interface Mechanic {
  ...
  commissionRates?: { servicePercent?: number; partPercent?: number }[];
}
...
export default function MechanicsPage() {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      toast.success("Mekanik/staf ditambahkan");
      setModalOpen(false); setForm({ name: "", email: "", phone: "", password: "", role: "MECHANIC" });
      fetchData();
        ...
        <CardContent className="p-0">
          <Table loading={loading} data={mechanics}
            columns={[
              ...
              { key: "commission", label: "Komisi", render: (m) => m.commissionRates?.[0] ? `${m.commissionRates[0].servicePercent || 0}% Jasa / ${m.commissionRates[0].partPercent || 0}% Part` : "-" },
            ]}
          />
        </CardContent>
      </Card>
        ...
        <form onSubmit={handleSave} className="space-y-4">
          {[
            ...
            <div key={field}>
              ...
              <input type={type || "text"} value={(form as any)[field]} onChange={(e) => setForm({ ...form, [field]: e.target.value })} required={required} className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm" />
            </div>
          ))}
          <div>
            <label className="block text-sm font-medium mb-1">Role</label>
            <select value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm">
